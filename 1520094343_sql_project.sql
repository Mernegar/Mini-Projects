...
select name as Facility_Name , firstname || ' ' || surname as Members_name,
...
and (( members.memid = 0 and guestcost * slots > 30) or
(members.memid > 0 and  membercost * slots > 30)) 
order by cost Desc;
...
select f.name as Name , firstname || ' ' || surname as member_Name , 
...
where ((b.memid = 0 and guestcost * slots > 30 ) or (b.memid > 0 and membercost * slots > 30)) and
...
select name , 
 ...
 group by name 
 having sum (case when bookings.memid = 0 then guestcost * slots else membercost * slots end ) < 1000
 order by revenue;
